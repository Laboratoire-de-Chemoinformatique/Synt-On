...
def main(inp, keepPG, output=None, Ro2Filtr=False):
    if not output:
        output = inp
    with open(inp) as inp, open(output + "_BBmode.smi", "w") as out, open(
        output + "_Synthmode.smi", "w"
    ) as outS, open(output + "_NotProcessed", "w") as notProc, open(
        output + "_NotClassified", "w"
    ) as notClassified:
        for line in inp:
            sline = line.strip()
            if sline:
                ...
                for molSmiles in line.split()[0].split("."):
                    ...
                    if ind2del:
                        for ind in ind2del[::-1]:
                            Classes.pop(ind)
                    azoles, fSynt = mainSynthonsGenerator(
                        Chem.MolToSmiles(initMol),
                        keepPG,
                        Classes,
                        returnBoolAndDict=True,
                    )
                    for synth1 in fSynt:
                ...
                if finalSynthon:
                    ...
                    for synth2 in finalSynthon:
                        if Ro2Filtr:
                            ...
                            outS.write(
                                synth2 + " " + "+".join(finalSynthon[synth2]) + " " + sline + "\n"
                            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="BBs classification and Synthons generation for large BBs libraries",
        ...
        prog="SyntOn_BBsBulkClassificationAndSynthonization",
    ...
    if args.nCores == -1:
        ...
        fixed_main = partial(main, keepPG=args.keepPG, Ro2Filtr=args.Ro2Filtr)
        nCores = args.nCores
        finalLog = []
        with ProcessPoolExecutor(max_workers=nCores) as executor:
            for out in executor.map(fixed_main, outNamesList):
                finalLog.append(out)
        with open(args.output + "_BBmode.smi", "w") as out, open(
            args.output + "_Synthmode.smi", "w"
        ) as outS, open(args.output + "_NotProcessed", "w") as notProc, open(
            args.output + "_NotClassified", "w"
        ) as notClassified:
        ...
        for line in inpS:
            ...
            if sline and len(sline.split()) == colNumb + 2:
                if sline.split()[0] not in synthons:
                    synthons[sline.split()[0]] = {
                        "Class": set(sline.split()[1].split(",")),
                        "BBs": {sline.split()[2]},
                        "BB_Ids": {sline.split()[3]},
                        "Count": 1,
                    }
                else:
