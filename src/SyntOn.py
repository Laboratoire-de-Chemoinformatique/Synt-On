...
class synthon:
    def __init__(
        ...
        directParent=None,
        directChildren=None,
        syntheticPathway=None,
        SynthLibProvided=False,
    ):
        self.smiles = smiles
        pat = re.compile("\[\w*:\w*\]")
        self.functionalityCount = len(
            [
                self.smiles[m.start() : m.start() + 2] + self.smiles[m.end() - 4 : m.end()]
                for m in re.finditer(pat, self.smiles)
            ]
        )
        self.marks = sorted(
            [
    ...
    def searchForSynthonAnalogues(self, synthLib: dict, simTh=-1):
        ...
        refMarksVallences = "+".join(
            sorted(
                [
                    atom.GetSymbol() + ":" + str(atom.GetTotalDegree())
        ...
        for synth in synthLib:
            if (
                self.marks == synthLib[synth]["marks"]
                and synth != self.smiles
                and refMarksVallences == synthLib[synth]["marksVallences"]
            ):
                if (
                    simTh != -1
                    and DataStructs.TanimotoSimilarity(queryFP, synthLib[synth]["fp_b"]) >= simTh
                ):
                    ...
                    if (
                        synthLib[synth]["n_atoms"] <= self.AtomNumbers + 1
                        and synthLib[synth]["n_atoms"] >= self.AtomNumbers - 1
                    ):
                        ...
                        if synthLib[synth]["n_atoms"] == self.AtomNumbers and sorted(
                            refList_qList + qList_refList
                        ) == ["c", "n"]:
                            ...
                            qList_refList
                            and len(qList_refList) == 1
                            and qList_refList[0] in posAnaloguesScreeningAtomsAllowed
                        ):
                            refMolRings = CalcNumRings(refMol)
                            if synthLib[synth]["n_rings"] == refMolRings:
                            ...
                            refList_qList
                            and len(refList_qList) == 0
                            and refList_qList[0] in posAnaloguesScreeningAtomsAllowed
                        ):
                            refMolRings = CalcNumRings(refMol)
                            if synthLib[synth]["n_rings"] == refMolRings:
                                analogMol = Chem.MolFromSmiles(synth)
    ...
    def printSynthonInfo(self):
        ...
        if self.SynthLibProvided:
            if self.correspondingBB:
                ...
                print(
                    "Synthon was not found in provided library of building blocks. "
                    + str(len(self.bbAnalogues))
                    + " analog(s) has/have been found"
                )
                print(
                    "BB analogues:\n"
        ...
        if self.directChildren:
            print(
                "Children synthons: " + ".".join(list(set([x.smiles for x in self.directChildren])))
            )
        else:
...
class syntheticPathway:
    def __init__(
        self,
        name,
        synthPathwayReactions,
        reagentsNumber,
        cutLevel,
        directParentsSynthPathways=None,
        SynthLibProvided=False,
    ):
        self.name = name
    ...
    def printShortReagentSetInfo(self):
        ...
        if self.SynthLibProvided:
            print(
                self.name
                + " "
                + ".".join([x.smiles for x in self.participatingSynthon])
                + " "
                + "Availability rate (% of atoms of fragmented molecule coming from available synthons): "
                + str(self.availabilityRate)
            )
        else:
            ...
            if self.directChildrenSynthPathways:
                ...
                for comb in self.directChildrenSynthPathways:
                    print(
                        ...
                        + comb.name
                        + " "
                        + "->".join(comb.synthPathwayReactions)
                        + " ||| "
                        + "Participating synthons: "
                        + ".".join([x.smiles for x in comb.participatingSynthon])
                    )
        ...
        if not strictAvailabilityMode:
            for ind, synth in enumerate(self.participatingSynthon):
                ...
                if "Reagent_" + str(ind + 1) not in synthonsDict:
                    synthonsDict["Reagent_" + str(ind + 1)] = {
                        "synthons": [],
                        "bivalentN": synth.bivalentN,
                    }
                ...
                if not synth.correspondingBB:
                    ...
                    SynthonsForAnaloguesSynthesis.add(
                        synth.smiles + " " + synth.correspondingBB + " originalBB\n"
                    )
...
class enumeration:
    ...
    def getReconstructedMols(
        self, allowedToRunSubprocesses=False, randomSeed=None, seed=(0, 0), mainRun=True
    ):
        pat = re.compile("\[\w*:\w*\]")
        if randomSeed == None:
        ...
        if randomSeed:
            allowedMarks = []
            for key in [
                Chem.MolToSmiles(randomSeed, canonical=True)[m.start() + 1]
                + ":"
                + Chem.MolToSmiles(randomSeed, canonical=True)[m.end() - 3 : m.end() - 1]
            ...
            for partner in self.__poliFuncBB + self.__biFuncBB + self.__monoFuncBB:
                partnerMarks = set(
                    [
                        Chem.MolToSmiles(partner, canonical=True)[m.start() + 1]
                        + ":"
                        + Chem.MolToSmiles(partner, canonical=True)[m.end() - 3 : m.end() - 1]
                ...
                if allowedMarks.intersection(partnerMarks):
                    if allowedToRunSubprocesses:
                        ...
                        queue = Queue()
                        process = Process(
                            target=self.__molReconsrtuction,
                            args=(
                                randomSeed,
                                partner,
                                1,
                        ...
                        Pool.append(
                            [process, queue, False]
                        )  # [the process, queue, was it already joined]
                        if self.__genNonUniqMols >= self.__desiredNumberOfNewMols:
                            break
                    else:
                        self.results.update(
                            self.__molReconsrtuction(randomSeed, partner, 1, queue=None)
                        )
                        print(
                            "Number of so far reconstructed unique molecules = "
        ...
        while mainRun:
            if not allowedToRunSubprocesses and len(self.results) >= self.__desiredNumberOfNewMols:
                ...
                allowedToRunSubprocesses and self.__genNonUniqMols >= self.__desiredNumberOfNewMols
            ):
                break
            whileCount += 1
            seed = list(seed)
            ...
            if randomSeed is None:
                break
            self.results.update(
                self.getReconstructedMols(allowedToRunSubprocesses, randomSeed, seed, mainRun=False)
            )
        d_names, f_names, main_dir = listDir(self.__outDir)
        with open(
            os.path.join(
                self.__outDir,
                "FinalOut_allEnumeratedCompounds_DuplicatesCanBePresent.smi",
            ),
            "ab",
        ) as out:
            for file in f_names:
    ...
    def AnaloguesGeneration(self):
        if type(self.__Synthons) == dict:
            for rid in range(len(self.__reactions)):
                ...
                for ind1 in range(1, len(self.__Synthons) + 1):
                    for ind2 in range(ind1 + 1, len(self.__Synthons) + 1):
                        ...
                        if self.__checkBB_reactionCombination(
                            self.__Synthons[Synthonset][0],
                            self.__Synthons[SynthonsetPartner][0],
                            reaction,
                        ):
                            for firstBB in self.__Synthons[Synthonset]:
                                for secondBB in self.__Synthons[SynthonsetPartner]:
                                    ...
                                    self.results.update(
                                        self.__molAnaloguesLibEnumeration(
                                            reagent=firstBB,
                                            partner=secondBB,
                                            numberOfBBalreadyReacted=1,
                                            SynthonsetsUsed=SynthonsetsUsed,
                                            reactionToUse=rid,
                                        )
                                    )
                                    if len(self.results) >= self.__desiredNumberOfNewMols:
            ...
            print(
                "Separate reconstructur should be evocken for Molecule analogues generation "
    ...
    def __molAnaloguesLibEnumeration(
        self,
        reagent,
        partner,
        numberOfBBalreadyReacted,
        SynthonsetsUsed,
        reactionToUse,
        usedReactions=None,
        firstLaunch=True,
                        ...
                        for rid in range(len(self.__reactions)):
                            if rid not in newUsedReactions:
                                ...
                                for SynthonsetPartner in self.__Synthons:
                                    if (
                                        SynthonsetPartner not in SynthonsetsUsed
                                        and self.__checkBB_reactionCombination(
                                            prod,
                                            self.__Synthons[SynthonsetPartner][0],
                                            reaction,
                                        )
                                        ...
                                        for secondBB in self.__Synthons[SynthonsetPartner]:
                                            ...
                                            subResults = self.__molAnaloguesLibEnumeration(
                                                reagent=prod,
                                                partner=secondBB,
                                                numberOfBBalreadyReacted=numberOfBBalreadyReacted
                                                + 1,
                                                SynthonsetsUsed=newSynthonsetsUsed,
                                                reactionToUse=rid,
                                                usedReactions=newUsedReactions,
                                                firstLaunch=False,
                                            )
                                            if subResults:
                                                allProducts.update(subResults)
                                            if (
                                                len(allProducts) >= self.__desiredNumberOfNewMols
                                                and firstLaunch
                                            ):
    ...
    def __molReconsrtuction(
        self,
        reagent,
        partner,
        numberOfBBalreadyReacted,
        alreadyUsedReactionsInds=None,
        queue=None,
    ):
                ...
                for prodSet in products:
                    for prod in prodSet:
                        ...
                        if functionality:
                            allowedMarks = []
                            for key in [
                                prodSMILES[m.start() + 1]
                                + ":"
                                + prodSMILES[m.end() - 3 : m.end() - 1]
                                for m in re.finditer(pat, prodSMILES)
                            ]:
                                allowedMarks.extend(self.__marksCombinations[key])
                            allowedMarks = set(allowedMarks)
                            if (
                                numberOfBBalreadyReacted + 1
                                <= self.__maxNumberOfReactedSynthons - functionality
                            ):
                                if (
                                    numberOfBBalreadyReacted + 1
                                    == self.__maxNumberOfReactedSynthons - functionality
                                    ...
                                    for newPartner in self.__monoFuncBB:
                                        partnerMarks = set(
                                            [
                                                Chem.MolToSmiles(newPartner, canonical=True)[
                                                    m.start() + 1
                                                ]
                                                + ":"
                                                + Chem.MolToSmiles(newPartner, canonical=True)[
                                        ...
                                        if allowedMarks.intersection(partnerMarks):
                                            ...
                                            subResults = self.__molReconsrtuction(
                                                prod,
                                                newPartner,
                                                numberOfBBalreadyReacted + 1,
                                    ...
                                    for newPartner in (
                                        self.__monoFuncBB + self.__biFuncBB + self.__poliFuncBB
                                    ):
                                        partnerMarks = set(
                                            [
                                                Chem.MolToSmiles(newPartner, canonical=True)[
        ...
        if Synthons == None:
            ...
            if analoguesEnumeration:
                ...
                for subset in Synthons:
                    ...
                    self.__Synthons[subset] = self.__PrepMolForReconstruction(
                        bbsList, Synthons[subset]["bivalentN"]
                    )
            else:
                monoFuncBB = []
                biFuncBB = []
                ...
                if monoFuncBB == None:
                    raise ValueError(
                        "No monofunctional building blocks are provided for enumeration"
                    )
                else:
                    self.__monoFuncBB = self.__PrepMolForReconstruction(
                        [Chem.MolFromSmiles(x) for x in monoFuncBB]
                    )
                    random.shuffle(self.__monoFuncBB)
                if biFuncBB:
                    self.__biFuncBB = self.__PrepMolForReconstruction(
                        [Chem.MolFromSmiles(x) for x in biFuncBB]
                    )
                    random.shuffle(self.__biFuncBB)
                if poliFuncBB:
                    self.__poliFuncBB = self.__PrepMolForReconstruction(
                        [Chem.MolFromSmiles(x) for x in poliFuncBB]
                    )
    ...
    def __countAndMergeActiveThreads(self, Pool):
        ...
        for p in Pool:
            if p[1].empty():
                ...
                print(
                    "Number of so far reconstructed molecules (may contain duplicates) = "
                    + str(self.__genNonUniqMols)
                )
                d_names, f_names, main_dir = listDir(self.__outDir)
                n = 0
                for file in f_names:
...
class fragmentation:
    def __init__(
        ...
        MaxNumberOfStages=5,
        FragmentsToIgnore=None,
        setupFile=os.path.join(
            os.path.split(os.path.split(os.path.realpath(__file__))[0])[0],
            "config",
            "Setup.xml",
        ),
        macroCycleSetupFile=os.path.join(
            os.path.split(os.path.split(os.path.realpath(__file__))[0])[0],
            "config",
            "SetupForMacrocycles.xml",
        ),
        FindAnaloguesOfMissingSynthons=False,
        parsedSynthLib=False,
        SynthLibrary=None,
        Ro2SynthonsFiltration=False,
    ):
        max_rec = 0x100000
        ...
        (
            self.__reactionMacroCycleSetup,
            self.__macroCyclicReactionsToWorkWith,
        ) = self.__getMacroCycleSetup(reactionsToWorkWith.replace("R", "MR"))
        forbiddenMarks = [
            {"[N:11]", "[c:10]"},
            {"[N:11]", "[c:20]"},
            {"[N:11]", "[O:20]"},
                    ...
                    for synth in allSyntheticPathways[comb].participatingSynthon:
                        if synth != allSynthons["InitMol"]:
                            self.__cutOneSynthonHierarchically(
                                allSynthons[synth.smiles],
                                allSyntheticPathways[comb],
                                allSynthons,
                                allSyntheticPathways,
                                cutLevel=2,
                            )
                            if (
                                self.__fragmentationMode == "one_by_one"
                                and allSyntheticPathways != startingCombintaions
                            ):
                                break
    ...
    def getReactionForReconstruction(self, reactionList=None):
        ...
        if reactionList:
            for key in reactionList:
                if "R14" not in key:
                    reactionForReconstruction.append(
                        self.__reactionSetup[key]["ReconstructionSMARTS"]
                    )
        else:
        ...
        for line in open(SynthLib):
            ...
            if sline:
                ...
                availableSynthons[sline.split()[0]]["marks"] = sorted(
                    [
                        sline.split()[0][m.start() : m.start() + 2]
                        + sline.split()[0][m.end() - 4 : m.end()]
                        for m in re.finditer(pat, sline.split()[0])
                    ]
                )
                availableSynthons[sline.split()[0]]["marksVallences"] = "+".join(
                    sorted(
                        [
                            atom.GetSymbol() + ":" + str(atom.GetTotalDegree())
                            for atom in mol.GetAtoms()
    ...
    def __getSetup(self, reactionsToWorkWith):
        ...
        if self.__fragmentationMode == "use_all":
            ...
            reaction_list = [
                ...
                if reaction not in self.__getReactionList(reactionsToWorkWith, allReactions)
            ]
    ...
    def __getMacroCycleSetup(self, reactionsToWorkWith):
        ...
        if self.__fragmentationMode == "use_all":
            ...
            reaction_list = self.__getReactionList(
                reactionsToWorkWith, allReactions, MacroCycles=True
            )
        elif self.__fragmentationMode == "block by block":
            reaction_list = []
            ...
            reaction_list = [
                ...
                for reaction in allReactions
                if reaction
                not in self.__getReactionList(reactionsToWorkWith, allReactions, MacroCycles=True)
            ]
        # reaction_list -> Ids of reactions that were specified by user to be used for the fragmentation
        return reactionSetup, reaction_list
                ...
                for ch in child:
                    for subCh in ch:
                        if subCh.get("SMARTS"):
                            ...
                            reactionSetup[subCh.tag]["ReconstructionSMARTS"] = subCh.get(
    ...
    def __firstMolCut(self, mol):
        ...
        allSynthons = {}
        allSyntheticPathways = {}
        allSyntheticPathways["zeroCombin"] = syntheticPathway(
            name="zeroCombin", reagentsNumber=0, synthPathwayReactions=None, cutLevel=0
        )
        allSynthons["InitMol"] = synthon(
            Chem.MolToSmiles(mol, canonical=True),
            syntheticPathway={allSyntheticPathways["zeroCombin"]},
            cutLevel=0,
        )
                ...
                for i in range(setsCount):
                    ...
                    for product in prodSet:
                        ...
                        for lablesSet in Labels.split("|"):
                            if macroCycle:
                                labledSynthon = self.__getMacroCycleLabledSmiles(
                                    product, lablesSet.split(";"), cutCount=None
                                )
                            else:
                                labledSynthon = self.__getLabledSmiles(
                                    product, lablesSet.split(";"), cutCount=None
                                )
                        if (
                            not labledSynthon
                            or labledSynthon.count(":")
                            > self.__maxNumberOfReactionCentersPerFragment
                            or (
                                CalcNumRings(Chem.MolFromSmiles(labledSynthon)) == 0
                                and labledSynthon.count(":")
                                > (Chem.MolFromSmiles(labledSynthon).GetNumHeavyAtoms() + 1) / 3
                            )
                            or (
                                CalcNumRings(Chem.MolFromSmiles(labledSynthon)) != 0
                                and labledSynthon.count(":")
                    ...
                    if synthonsProdSet:
                        ...
                        if reagSetKey not in allSyntheticPathways:
                            if self.SynthLib:
                                allSyntheticPathways[reagSetKey] = syntheticPathway(
                                    name=reagSetName,
                                    reagentsNumber=len(prodSet),
                                    synthPathwayReactions=[reactionSetup[rId]["Name"]],
                                    cutLevel=1,
                            ...
                            allSyntheticPathways[reagSetKey].directParentsSynthPathways.append(
                                allSyntheticPathways["zeroCombin"]
                            )
                        ...
                        for synth in synthonsProdSet:
                            ...
                            elif allSyntheticPathways[reagSetKey] not in set(
                                allSynthons[synth].syntheticPathway
                            ):
                                allSynthons[synth].syntheticPathway.append(
                                    allSyntheticPathways[reagSetKey]
                                )
                            allSyntheticPathways[reagSetKey].participatingSynthon.append(
                                allSynthons[synth]
                            )
                            if self.__fragmentationMode == "one_by_one":
    ...
    def __cutOneSynthonHierarchically(
        self, synthOld, oldComb, allSynthons, allSyntheticPathways, cutLevel
    ):
                ...
                for i in range(setsCount):
                    ...
                    for product in prodSet:
                        if (
                            product.GetNumHeavyAtoms() < 3
                            or (
                                "[V]" in Chem.MolToSmiles(product, canonical=True)
                                and product.GetNumHeavyAtoms() < 4
                            )
                            or Chem.MolToSmiles(product, canonical=True) in self.__SmilesToIgnore
                        ):
                            Ignore = True
                    ...
                    for product in prodSet:
                        ...
                        if (
                            Chem.MolToSmiles(product) == "*c1c(C[NH:20]C)nn(C)c1C#N"
                            or Chem.MolToSmiles(product)
                            == "*c1cnc(N)c(OC(C)c2cc(F)ccc2[CH:10]=O)c1"
                        ):
                            print("#################################")
                            print(labledSynthon)
                        if not labledSynthon:
                            ...
                            labledSynthon.count(":") > self.__maxNumberOfReactionCentersPerFragment
                            or (
                                CalcNumRings(Chem.MolFromSmiles(labledSynthon)) == 0
                                and labledSynthon.count(":")
                                > (Chem.MolFromSmiles(labledSynthon).GetNumHeavyAtoms() + 1) / 3
                        ...
                        marks = frozenset(
                            [
                                labledSynthon[m.start() : m.start() + 2]
                                + labledSynthon[m.end() - 4 : m.end()]
                                for m in re.finditer(pat, labledSynthon)
                            ]
                        )
                    ...
                    if synthonsProdSet:
                        if synthOld.marks:
                            ...
                            for labledSynthon in synthonsProdSet:
                                ...
                                if (
                                    len(marksNew) == len(synthOld.marks)
                                    and marksNew != synthOld.marks
                                ):
                            ...
                            if 1 not in set(marksCheckPrev):
                                ...
                                checkTotalMarks = [
                                    1 for mark in synthOld.marks if mark not in allNewMarks
                                ]
                                if checkTotalMarks:
                                    if labledSynthon == "Cn1nc(C[NH:20]C)[cH:21]c1C#N":
                                        print("Continue 2")
                        ...
                        reagSetKeyList.extend(
                            [
                                synth.smiles
                                for synth in oldComb.participatingSynthon
                                if synthOld != synth
                            ]
                        )
                        reagSetKeyList.sort()
                        ...
                        if reagSetKey not in allSyntheticPathways:
                            ...
                            if self.__reactionSetup[rId]["Name"] not in set(
                                allSyntheticPathways[reagSetKey].synthPathwayReactions
                            ):
                                allSyntheticPathways[reagSetKey].synthPathwayReactions.append(
                                    self.__reactionSetup[rId]["Name"]
                                )
                            oldComb.directChildrenSynthPathways.append(
                                allSyntheticPathways[reagSetKey]
                            )
                        elif (
                            oldComb.name != reagSetName
                            and allSyntheticPathways[reagSetKey]
                            not in oldComb.directParentsSynthPathways
                        ):
                            allSyntheticPathways[reagSetKey].directParentsSynthPathways.append(
                                oldComb
                            )
                        ...
                        if (
                            "[NH2:20]" in kidsMarksList
                            and parentMarksList.count("[NH:20]")
                            == kidsMarksList.count("[NH:20]") + 1
                        ):
    ...
    def __getMacroCycleLabledSmiles(self, productMolecule: Chem.rdchem.Mol, Labels: list, cutCount):
        ...
        for label in Labels:
            if productSmiles.find(label.split("->")[0]) != -1:
                ...
                if labeledSmiles:
                    if "*" in labeledSmiles:
                        labeledSmiles = self.__getMacroCycleLabledSmiles(
                            Chem.MolFromSmiles(labeledSmiles), Labels, cutCount
                        )
        ...
        if SyntOnfragmentor.SynthLib != None:
            ...
            for comb in firstSynthons:
                comb.checkAvailability(
                    SyntOnfragmentor.SynthLib,
                    simTh,
                    SyntOnfragmentor.FindAnaloguesOfMissingSynthons,
                )
        return allSyntheticPathways, allSynthons
    else:
        return None, None
...
def analoguesLibraryGeneration(
    Smiles_molNameTuple,
    SyntOnfragmentor,
    outDir,
    simTh=-1,
    ...
    with open(
        os.path.join(
            outDir,
            "SynthonsForAnalogsGenerationForMol" + str(Smiles_molNameTuple[1]) + ".smi",
        ),
        "w",
    ) as outSynthons:
        allSyntheticPathways, allSynthons = fragmentMolecule(
            Smiles_molNameTuple[0], SyntOnfragmentor, simTh=simTh
        )
        ...
        if allSyntheticPathways and len(allSyntheticPathways) > 1:
            ...
            for comb in leafsComb:
                ...
                (
                    synthonsDict,
                    SynthonsForAnaloguesSynthesisLocal,
                ) = comb.getSynthonsForAnaloguesGeneration(
                    SyntOnfragmentor.SynthLib,
                    simTh,
                ...
                if synthonsDict and SynthonsForAnaloguesSynthesisLocal:
                    ...
                    reactionsUsedInFragmentationReactions = [
                        rid.split("_")[0] for rid in comb.name.split("|")
                    ]
            ...
            if not CompletePath:
                ...
                shortestSynthesis = findShortestSynthPathWithAvailableBBlib(
                    synthonsAfterOneCut, showAll=False, firstLaunch=True
                )
                for comb in shortestSynthesis:
                    if comb.availabilityRate == 1.0:
                        ...
                        if synthonsDict and SynthonsForAnaloguesSynthesisLocal:
                            ...
                            reactionForReconstruction = (
                                SyntOnfragmentor.getReactionForReconstruction(
                                    reactionsUsedInFragmentationReactions
                                )
                            )
                            enumerator = enumeration(
                                ...
                                reactionSMARTS=reactionForReconstruction,
                                maxNumberOfReactedSynthons=len(synthonsDict),
                                desiredNumberOfNewMols=1000000,
                                nCores=1,
                                analoguesEnumeration=True,
                            )
                            # reconstructedMols.update(enumerator.newAnaloguesGeneration())
            ...
            with open(
                os.path.join(outDir, "AnalogsForMol" + str(Smiles_molNameTuple[1]) + ".smi"),
                "w",
            ) as out:
...
def findShortestSynthPathWithAvailableBBlib(
    firstSynthons: list, allCombSynthons=None, firstLaunch=True, showAll=True
):
    max_rec = 0x100000
    # May segfault without this line. 0x100 is a guess at the size of each stack frame.
    resource.setrlimit(resource.RLIMIT_STACK, [0x100 * max_rec, resource.RLIM_INFINITY])
    ...
    for comb in firstSynthons:
        ...
        if comb.directChildrenSynthPathways:
            findShortestSynthPathWithAvailableBBlib(
                comb.directChildrenSynthPathways, allCombSynthons, firstLaunch=False
            )
    if firstLaunch:
        ...
        for comb in allCombSynthons:
            if comb.availabilityRate == rate and comb.reagentsNumber == reagents:
                for savedComb in shortestAvailablePathways:
                    if set([x.smiles for x in comb.participatingSynthon]) == set(
                        [x.smiles for x in savedComb.participatingSynthon]
                    ):
                        add = False
                        break
                if add:
        ...
        print(
            str(len(shortestAvailablePathways))
            + " equivalent synthetic pathway(s) have been found."
        )
        return shortestAvailablePathways
