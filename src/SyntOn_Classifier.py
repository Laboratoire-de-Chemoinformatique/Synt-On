...
def BBClassifier(molSmiles=None, mol=None):
    ...
    for bigClass in SmartsLib:
        for subClass in SmartsLib[bigClass]:
            if __classChecker(
                SmartsLib[bigClass][subClass]["ShouldContainAtLeastOne"],
                SmartsLib[bigClass][subClass]["ShouldAlsoContain"],
                SmartsLib[bigClass][subClass]["shouldNotContain"],
                mol,
            ):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Classification of building blocks. Separates provided library into several sublibraries according to the reagents classess.",
