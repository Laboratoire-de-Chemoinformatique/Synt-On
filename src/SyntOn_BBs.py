...
def mainSynthonsGenerator(
    initSmi, keepPG=False, Classes=None, returnDict=False, returnBoolAndDict=False
):
    solventsToIgnore = [
        "OC(=O)C(=O)O",
        "CC(=O)O",
    ...
    canonicalSmilesOfSolventsToIgnore = set(
        [Chem.MolToSmiles(Chem.MolFromSmiles(x), canonical=True) for x in solventsToIgnore]
    )
    ...
    if initMol == None or initMol.HasSubstructMatch(query):
        ...
        if Classes == None:
            ...
            Classes = [
                clas for clas in AllClasses if "MedChemHighlights" not in clas and "DEL" not in clas
            ]
        BBmarks = os.path.join(
            ...
            if "Bifunctional" in Classes[ind] or "Trifunctional" in Classes[ind]:
                ...
                if polyfunc and "Bifunctional" not in Classes[ind]:
                    ...
                    for subName in [y for x in polyfuncName for y in x.split("_")]:
                        if (
                            subName in Classes[ind]
                            and "Bifunctional_NbnDi_Amines" not in polyfuncName
                        ):
                ...
                for mol in molsToWorkWith:
                    synthons = __synthonsAssignement(
                        Classes[ind],
                        molsToWorkWith[mol],
                        mol,
                        MarksSetup,
        ...
        if not finalSynthon and "Esters_Esters" in Classes:
            ...
            finalSynthon = __NormalSynthonsGenerator(
                LabelList, ReactionLIST, set(), "Esters_Esters", initMol, func=1
            )
            ...
            for synthon in finalSynthon:
                if "Ketones_Ketones" in finalSynthon[synthon]:
                    ...
                    for cls in newClasses:
                        if "Alcohols" in cls:
                            nAzoles, nFinalSynthon = mainSynthonsGenerator(
                                synthon, keepPG, [cls], returnBoolAndDict=True
                            )
                            for newSynth in nFinalSynthon:
    ...
    if returnDict:
        ...
        print(
            "All generated synthons ("
            + str(len(finalSynthon))
            + "): "
            + ".".join([x for x in finalSynthon])
        )
...
def __synthonsAssignement(
    CurrentClass, PreviousClasses, molSmi, MarksSetup, keepSynthonsWithPG=True
):
    additionalBifuncClasses = [
    ...
    if CurrentClass in PolymerReagents:
        ...
        for i in range(len(MarksSetup[CurrentClass]["Labels"].split("|"))):
            synthons = __SynthonsGeneratorsForPolymerReagents(
                MarksSetup[CurrentClass]["Labels"].split("|")[i],
                MarksSetup[CurrentClass]["SMARTS"].split("|")[i],
                CurrentClass,
                MolsToWorkWith,
            )
            if synthons:
        ...
        synthons = __ProtectiveGroupRemoval(
            MarksSetup[CurrentClass]["Labels"],
            MarksSetup[CurrentClass]["SMARTS"],
            mol,
            keepSynthonsWithPG,
            firstReactionAsPrep,
            func,
        ...
        synthons = __FirstReactionAsPrep(
            MarksSetup[CurrentClass]["Labels"],
            MarksSetup[CurrentClass]["SMARTS"],
            PreviousClasses,
            CurrentClass,
        ...
        synthons = __NormalSynthonsGenerator(
            MarksSetup[CurrentClass]["Labels"],
            MarksSetup[CurrentClass]["SMARTS"],
            PreviousClasses,
            CurrentClass,
...
def __azolesSynthonPostGeneration(labledSynthons):
    ...
    for molSmiles in labledSynthons:
        marksPrevious = [
            molSmiles[m.start() : m.start() + 2] + molSmiles[m.end() - 4 : m.end()]
            for m in re.finditer(pat, molSmiles)
        ]
                ...
                for product in productSet:
                    ...
                    if marksPrevious:
                        for synth in labledSynthon:
                            marksNew = [
                                synth[m.start() : m.start() + 2] + synth[m.end() - 4 : m.end()]
                                for m in re.finditer(pat, synth)
                            ]
...
def __SynthonsGeneratorsForPolymerReagents(
    Label,
    rule,
    Class,
    MolsToWorkWith,
...
def __ProtectiveGroupRemoval(
    ...
    ReactionLISTBeforePGRemoval = ReactionLIST.split("|")[
        : len(LabelsLISTBeforePGRemoval.split("|"))
    ]
    firstStopInd = len(LabelsLISTBeforePGRemoval.split("|"))
    finalSynthons = {}
    if firstReactionAsPrep:
        synthonsBeforeFirstPGremoval = __FirstReactionAsPrep(
            LabelsLISTBeforePGRemoval,
        ...
        if "Ester" in CurrentClass and "Acid" in CurrentClass and func == 3 and not twoPGs:
            synthonsBeforeFirstPGremoval = __NormalSynthonsGenerator(
                LabelsLISTBeforePGRemoval,
                "|".join(ReactionLISTBeforePGRemoval),
                PreviousClasses,
                CurrentClass,
    ...
    if CurrentClass == "Trifunctional_Di_Esters_Amino":
        ...
        SynthonsWithoutPG = __SynthonsGeneratorsForPolymerReagents(
            LabelsLIST.split("|")[2],
            ReactionLIST.split("|")[2],
            CurrentClass,
            synthonsBeforeFirstPGremoval,
            Deprotection=True,
        )
        ...
        lastSynthons = __SynthonsGeneratorsForPolymerReagents(
            LabelsLIST.split("|")[3],
    ...
    if (
        keepSynthonsWithPG
        or PGremovalRule
        == "[N;+0,+1;$([N+](=O)([#6])[O-]),$(N(=O)([#6])=O):1](=[O:2])=,-[O;+0,-1:3]>>[NH2,+0:1]"
    ):
    ...
    for smi in synthonsBeforeFirstPGremoval:
        if (
            ...
            and not twoPGs
            and smi.count(":") < 2
            and "Ester" not in CurrentClass
            and "AlkyneCH" not in CurrentClass
        ):  # this force algorythm to use for the PG removal step only synthons where all unprotected functional groups has been transformed into synthons
            continue
    ...
    ReactionLISTBetweenPGRemoval = ReactionLIST.split("|")[
        len(LabelsLISTBeforePGRemoval.split("|"))
        + 1 : len(LabelsLISTBeforePGRemoval.split("|"))
        + len(LabelsLISTBetweenPGRemoval.split("|"))
        + 1
    ]
    synthonsBetweenPGremoval = SynthonsWithoutPG.copy()
    for newSynthon in SynthonsWithoutPG:
        newSynthonsBetweenPGremoval = __NormalSynthonsGenerator(
            LabelsLISTBetweenPGRemoval,
            "|".join(ReactionLISTBetweenPGRemoval),
            PreviousClasses,
            CurrentClass,
            Chem.MolFromSmiles(newSynthon),
            func=1,
        )
    ...
    if (
        len(LabelsLIST.split("|No|")) == 2
        or keepSynthonsWithPG
        or PGremovalRule
        == "[N;+0,+1;$([N+](=O)([#6])[O-]),$(N(=O)([#6])=O):1](=[O:2])=,-[O;+0,-1:3]>>[NH2,+0:1]"
    ):
        for synth in SynthonsWithoutPG:
    ...
    if len(LabelsLIST.split("|No|")) == 3:
        ...
        ReactionLast = ReactionLIST.split("|")[
            len(LabelsLISTBeforePGRemoval.split("|"))
            + 1
            + len(LabelsLISTBetweenPGRemoval.split("|"))
            + 1 :
        ...
        for newSynthon in SynthonsWithout2PG:
            lastSynthons = __NormalSynthonsGenerator(
                LabelsLast,
                "|".join(ReactionLast),
                PreviousClasses,
                CurrentClass,
...
def __NormalSynthonsGenerator(
    LabelsLIST, ReactionLIST, PreviousClasses, CurrentClass, mol, func=1, usedInds=None
):
    if usedInds == None:
        usedInds = []
    labledSynthons = {}
            ...
            for productSet in products:
                for product in productSet:
                    ...
                    if func == 2:
                        ...
                        usedInds.append(ind)
                        newSynthons = __NormalSynthonsGenerator(
                            LabelsLIST,
                            ReactionLIST,
                            PreviousClasses,
                            CurrentClass,
...
def __FirstReactionAsPrep(LabelsLIST, ReactionLIST, PreviousClasses, CurrentClass, mol, func):
    ...
    if not products:
        if len(LabelsLIST.split("|")) > 1:
            return __FirstReactionAsPrep(
                "|".join(LabelsLIST.split("|")[1:]),
                "|".join(ReactionLIST.split("|")[1:]),
                PreviousClasses,
                CurrentClass,
                mol,
                func,
        ...
        if not synthonsAsInpForTheNextStep and "Boronics" in CurrentClass:
            ...
            for synth in synthonsAsInpForTheNextStep:
                labledSynthons = __NormalSynthonsGenerator(
                    "|".join(LabelsLIST.split("|")[1:]),
                    "|".join(ReactionLIST.split("|")[1:]),
                    PreviousClasses,
                    CurrentClass,
                    Chem.MolFromSmiles(synth),
                    func=func - 1,
                )
                if labledSynthons:
                    for synth in labledSynthons:
...
def generateScaffoldForBB(smiles, returnObjects=False):
    ...
    mol = None
    PGdict = {
        "NCbz": "[N:1][C;$(C(=O)O[CH2]c1[cH][cH][cH][cH][cH]1):2]>>[N:1]",
