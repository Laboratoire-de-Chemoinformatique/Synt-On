...
def main(
    ...
    if simTh == -1:
        ...
        simBBselection = True
    SyntOnfragmentor = fragmentation(
        ...
        FragmentsToIgnore=SmilesToIgnore,
        SynthLibrary=SynthLibrary,
        FindAnaloguesOfMissingSynthons=simBBselection,
    ...
    if analoguesLibGen and nCores == -1:
        ...
        fixed_analogsGenerationFunction = partial(
            analoguesLibraryGeneration,
            outDir=outDir,
            simTh=simTh,
            SyntOnfragmentor=SyntOnfragmentor,
            strictAvailabilityMode=strictAvailabilityMode,
            desiredNumberOfNewMols=desiredNumberOfNewMols,
        )
        nCores = args.nCores
            ...
            if line.strip():
                ...
                if allReagentSets and len(allReagentSets) > 1:
                    ...
                    shortestSynthesis = findShortestSynthPathWithAvailableBBlib(
                        fsynthonsAfterOneCut, showAll=False, firstLaunch=True
                    )
                    outOnePath.write(
                        line.strip()
                        + " "
                        + ".".join([x.smiles for x in shortestSynthesis[0].participatingSynthon])
                        + " "
                        + shortestSynthesis[0].name
                        + " "
                        + str(shortestSynthesis[0].reagentsNumber)
                        + " "
                        + str(shortestSynthesis[0].availabilityRate)
                        + " "
                    )
                    outOnePath.flush()
                    availableSynthons = {}
                    ...
                    outOnePath.write(
                        "AvailableSynthons:"
                        + "|".join(
                            [synth + "->" + availableSynthons[synth] for synth in availableSynthons]
                        )
                    )
                    outOnePath.flush()
                    ...
                    outAllSynthons.write(
                        line.strip() + " AvailableSynthons:" + ".".join(availableSynthons)
                    )
                    if notAvaialableSynthons:
                        outAllSynthons.write(
                            " notAvaialableSynthons:" + ".".join(notAvaialableSynthons)
                        )
                    outAllSynthons.write("\n")
        ...
        enumerator = enumeration(
            ...
            maxNumberOfReactedSynthons=MaxNumberOfStages + 1,
            MWupperTh=MWupperTh,
            MWlowerTh=MWlowerTh,
            desiredNumberOfNewMols=desiredNumberOfNewMols,
            nCores=nCores,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Compound fragmentaitiona and analogues generation.",
        ...
        prog="SyntOn_BulkFragmentationEnumerationAndAnaloguesDesign",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Library of available synthons. Generated from avaialable BBs using SyntOn_BBsBulkClassificationAndSynthonization.py",
    )
    parser.add_argument(
        "--nCores",
        default=-1,
        type=int,
        help="Number of CPUs available for parallelization.",
    )
    parser.add_argument(
        "--simTh",
        default=-1,
        type=float,
        help="Similarity threshold for BB analogues search. "
        "If not specified, only positional variational approach will be used for BBs search",
    )
    parser.add_argument(
        "--analoguesLibGen",
        action="store_true",
        help="Generate library of analogues from input mol",
    )
    parser.add_argument(
        "--strictAvailabilityMode",
        action="store_true",
        help="Only fully synthesizable analogues are generated. "
    ...
    parser.add_argument(
        "--Ro2Filtration",
        action="store_true",
        help="Filter input synthons library by Ro2 (MW <= 200, logP <= 2, H-bond donors count <= 2 and H-bond acceptors count <= 4)",
    )
    parser.add_argument(
        "--fragmentationMode",
        default="use_all",
        type=str,
        help="Mode of fragmentation (defines how the reaction list is specified)"
    ...
    parser.add_argument(
        ...
        default="R1-R13",
        type=str,
        help="List of RiDs to be used." "\n(default: R1-R13 (all reactions) ",
    )
    parser.add_argument(
        "--desiredNumberOfNewMols",
        default=1000,
        type=int,
    ...
    parser.add_argument(
        ...
        default=5,
        type=int,
        help="Maximal number of stages during fragmentation or enumeration." "\n(default: 5)",
    )
    parser.add_argument(
        "--maxNumberOfReactionCentersPerFragment",
        default=3,
        type=int,
        help="Maximal number of reaction centers per fragment." "\n(default: 3)",
    )
    parser.add_argument(
        "--enumerationMode",
        action="store_true",
        help="Enumerate library using input synthons",
    )
    parser.add_argument(
        "--MWupperTh",
        default=1000,
    ...
    if args.nCores == -1 or args.analoguesLibGen or args.enumerationMode:
        main(
            ...
            args.outDir,
            args.simTh,
            args.strictAvailabilityMode,
            args.nCores,
            args.analoguesLibGen,
            args.Ro2Filtration,
            fragmentationMode=args.fragmentationMode,
            ...
            enumerationMode=args.enumerationMode,
            MWupperTh=args.MWupperTh,
            MWlowerTh=args.MWlowerTh,
        )
    else:
        ...
        fixed_main = partial(
            ...
            outDir=args.outDir,
            simTh=args.simTh,
            SynthLibrary=args.SynthLibrary,
            strictAvailabilityMode=args.strictAvailabilityMode,
        ...
        with ProcessPoolExecutor(max_workers=nCores) as executor:
            for out in executor.map(fixed_main, outNamesList):
                finalLog.append(out)
        with open(args.input + "_out", "w") as outOnePath, open(
            "allSythons_" + args.input + "_out", "w"
        ) as outAllSynthons:
